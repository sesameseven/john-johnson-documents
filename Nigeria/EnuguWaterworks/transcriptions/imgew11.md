## Page11
← Previous: [Page 10](imgew10.md); Next: → [Page 12](imgew12.md); Back to: [Index](../combined.md)

The consumption of the Native Town is apparently therefore
some 95,000 gallons a day, which based on a population of 12,000
gives an average of 7.93 gallons per head per day.

The rate per head of adult native population works out at
5d. per annum as compared with the average rate paid by
Europeans which is £2 8s. per annum.

The figure quoted for population may vary and requires con-
firmation, and the capacity of the Native Town to pay is also a
matter which requires investigation.

Detail costs of the various sections of the works together with
unit costs are shown in Appendix " B".

---

**APPENDIX "A"**

CHEMICAL ANALYSIS OF A SAMPLE OF WATER FROM THE NGWO SPRINGS.

| Result of Analysis. | Parts per 100,000 |
| ------------------- | ------------------:|
| Saline Ammonia | 0.002 |
| Albuminoid Ammonia | 0.002 |
| Nitrite | Nil |
| Nitrate | Nil |
| Total Solids | 6.8 |
| Oxygen absorbed laboratory temp. in 3 hours | 0.0017 |
| Total Hardness | 5.0 |
| Sulphate (SO4) | Nil |
| Chlorine | 0.3 |
| Iron | Trace |
| Lime (CaO) | 2.2 |
| Magnesia (MgO) | Trace |

Analyst's Remarks:-

1. The above figures indicate that the water is suitable for domestic use.
2. The water contains nothing in solution which would act on steel.
3. The figure for total hardness is probably high.

| **A. B. Hobson,** |
|--------:|
|Government Analyst,|    
|Yaba.   |
|10th July, 1928.|   

← Previous: [Page 10](imgew10.md); Next: → [Page 12](imgew12.md); Back to: [Index](../combined.md)


